...
class SCRUB(BaseMethod):
    def __init__(self,
        ...
        for images, labels in self.train_retain_loader_img:
            print("train_retain_loader_synth_img:", images.shape, labels.shape)
            break

        for images, labels in self.train_fgt_loader_img:
            print("train_fgt_loader_synth_img:", images.shape, labels.shape)
            break
    ...
    def run(self):
        ...
        retain_features_train, retain_labels_train = flatten_loader(self.train_retain_loader_img)
        forget_features_train, forget_labels_train = flatten_loader(self.train_fgt_loader_img)

        teacher_remaining=self.teacher
        student_remaining=self.student
        retainfull_loader_val=self.retainfull_loader_real
        forgetfull_loader_val=self.forgetfull_loader_real
        retaintest_loader_val=self.test_retain_loader_img
        forgettest_loader_val=self.test_fgt_loader_img
        alpha=0.45
        gamma=0.45
        betha=0.1
        ...
        student_remaining.to(opt.device)
        teacher_remaining.to(opt.device)
        optimizer = optim.Adam(student_remaining.parameters(), lr=opt.lr_unlearn)
        loss_ce = nn.CrossEntropyLoss()
        for param in teacher_remaining.parameters():
            param.requires_grad = False
        
        for param in student_remaining.parameters():
            param.requires_grad = True
            
        for name, param in teacher_remaining.named_parameters():
            print(f"Teacher {name}: requires_grad={param.requires_grad}")

        for name, param in student_remaining.named_parameters():
        ...
        student_remaining.eval()

        retain_accuracy = evaluate_embedding_accuracy(student_remaining, self.train_retain_loader, opt.device)/100
        forget_accuracy = evaluate_embedding_accuracy(student_remaining, self.train_fgt_loader, opt.device)/100
        retaintest_val_acc = evaluate_embedding_accuracy(merged_model, self.test_retain_loader_img, opt.device)/100
        forgettest_val_acc = evaluate_embedding_accuracy(merged_model, self.test_fgt_loader_img, opt.device)/100
        retainfull_val_acc = evaluate_embedding_accuracy(merged_model.fc, self.retainfull_loader_real, opt.device)/100
        forgetfull_val_acc = evaluate_embedding_accuracy(merged_model.fc, self.forgetfull_loader_real, opt.device)/100

        AUS = calculate_AUS(forgettest_val_acc, retaintest_val_acc, a_or_value)
        ...
        for epoch in range(opt.epochs_unlearn):
            ...
            student_remaining.train()
            optimizer.zero_grad()

            retain_logits_student = student_remaining(retain_features_train)
            forget_logits_student = student_remaining(forget_features_train) 
            
            with torch.no_grad():
                retain_logits_teacher = teacher_remaining(retain_features_train) 
                forget_logits_teacher = teacher_remaining(forget_features_train)
            ...
            AUS = calculate_AUS(forgettest_val_acc, retaintest_val_acc, a_or_value)  # Calculate AUS using the accuracies
