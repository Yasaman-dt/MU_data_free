...
stats_df = pd.read_csv("results/results_mean_std_all_numeric.csv")
...
def get_data_free_flags(method, source):
    if method in ["original", "retrained"]:
        return ("--", "--")
    elif method in ["FineTuning","RetrainedEmbedding"]:
        return (r"\cmark", r"\cmark") if source == "synth" else (r"\xmark", r"\cmark")
    elif method in ["NegativeGradient", "RandomLabels", "BoundaryShrink", "BoundaryExpanding", "LAU"]:
        return (r"\cmark", r"\cmark") if source == "synth" else (r"\cmark", r"\xmark")
    elif method in ["NGFT_weighted", "DUCK", "SCRUB", "SCAR"]:
...
method_name_and_ref = {
    ...
    "RetrainedEmbedding": ("Retrained FC", r"–"),
    "FineTuning": ("Fine Tuning", r"\citep{golatkar2020eternal} Ours"),
    "NegativeGradient": ("Neg. Grad.", r"\citep{golatkar2020eternal} Ours"),
    "NGFT_weighted": ("Neg. Grad. +", r"\citep{golatkar2020eternal} Ours"),
    "RandomLabels": ("Rand. Lab.", r"\citep{hayase2020selective} Ours"),
    "BoundaryShrink": ("Boundary S.", r"\citep{chen2023boundary} Ours"),
    "BoundaryExpanding": ("Boundary E.", r"\citep{chen2023boundary} Ours"),
...
for dataset in datasets:
    ...
    # Define method order
    method_order = ["original", "retrained", "RetrainedEmbedding", "FineTuning", "NegativeGradient", "NGFT_weighted", "RandomLabels","BoundaryShrink", "BoundaryExpanding", "LAU", "SCRUB", "DUCK", "SCAR"]
    ...
    for idx, (_, row) in enumerate(dataset_rows.iterrows()):
        ...
        if prev_method_base in ["FineTuning", "BoundaryExpanding"]:
    ...
    # Save to file
    filename = f"results/table_{dataset.lower()}.tex"
...
for idx, key in enumerate(sorted(grouped_methods.keys(), key=sort_key)):
    ...
    if prev_base_method and base_method != prev_base_method:
        ...
        if prev_base_method in ["FineTuning", "BoundaryExpanding"]:
            latex_table += r"\midrule" + "\n"
...
# === Save to file (UTF-8)
with open("results/table_total.tex", "w", encoding="utf-8") as f:
    f.write(latex_table)
