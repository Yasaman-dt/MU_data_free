...
sources = [
    ...
    ("results_new/results_sigma5_confidence0/results_synth/samples_per_class_5000", None, "synth"),
    ("results_new/results_sigma5_confidence0.97/results_synth/samples_per_class_5000", None, "synth"),
    ("results_new/results_sigma50_confidence0/results_synth/samples_per_class_5000", None, "synth"),
    ("results_new/results_sigma50_confidence0.97/results_synth/samples_per_class_5000", None, "synth")
]
...
original_summary.to_csv("results_sigma/original_averaged_results.csv", index=False)
...
output_path = "results_new/results_retrained.csv"
    ...
    for method in methods:
        ...
        for file_path in files:
            ...
            if match:
                ...
                if sigma is None:
                    sigma_match = re.search(r"sigma(\d+(?:\.\d+)?)", folder_name)
                    sigma_val = float(sigma_match.group(1)) if sigma_match else None
                else:
                    sigma_val = sigma
                
                conf_match = re.search(r"confidence(\d+(?:\.\d+)?)", folder_name)
                conf_val = float(conf_match.group(1)) if conf_match else None
                
                df["sigma"] = sigma_val
                df["confidence"] = conf_val
...
if all_data:
    ...
    final_df.to_csv(os.path.join(parent_dir, "results_new/results_unlearning.csv"), index=False)
    ...
    # Save results
    best_df.to_csv(os.path.join(parent_dir, "results_new/results_unlearning_best_per_model_by_aus.csv"), index=False)
    ...
    save_dir = os.path.join(parent_dir, "results_new/best_per_dataset_method_source")
    ...
    combined_df.to_csv("results_new/results_total.csv", index=False)
    ...
    stats_path = os.path.join(parent_dir, "results_new/results_mean_std_all_numeric.csv")
